...
def printWifiDetails():
    nmap_out = subprocess.run(args = ['nmap', '-sP', '--system-dns', '192.168.0.255/24'],
                              universal_newlines = True,
                              stdout = subprocess.PIPE)

    nmap_lines = nmap_out.stdout.splitlines()
    print(nmap_out)
    #nmap = subprocess.run("nmap"," -sP 192.168.0.255/24", capture_output=True)
    data = {
        "time" : time.time(),
        "event": "PiEvent",
        "value": nmap_out.stdout.splitlines()
        }
    print(data)
...
python_object = printWifiDetails()
